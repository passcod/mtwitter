...
// Don’t export the pkginfo values,
// but have them available for use here.
var pkginfo = {exports: []};
require('pkginfo')(pkginfo);
pkginfo = pkginfo.exports;
...
Twitter.version = pkginfo.version;
...
Twitter.prototype.post = function(url, content, content_type, callback) {
  ...
  var handle = function(error, data) {
    if (error && error.statusCode) {
      var err = new Error(
        'HTTP Error ' +
        error.statusCode + ': ' +
...
Twitter.prototype.stream = function(method, params, callback) {
  ...
  // Iterate on params properties, if any property is an array,
  ...
  request.on('response', function(response) {
    ...
    if(response.statusCode > 200) {
      ...
      // FIXME: Somehow provide chunks of the response
      //        when the stream isconnected

      // Pass HTTP response data to the parser,
      // which raises events on the stream
      ...
      // socket. So, 'stream' object will fire the 'destroy' event as we can
      // see above.
...
Twitter.prototype.login = function(mount, success) {
  ...
  return function handle(req, res, next) {
    ...
    if (!self.oauth._authorize_callback) {
      ...
      var scheme = 'http://';
      if (req.socket.secure || self.options.secure) {
        scheme = 'https://';
      }
    ...
    var returningFromTwitter = path.query && path.query.oauth_token &&
        path.query.oauth_verifier && twauth && twauth.oauth_token_secret;
    ...
    if (twauth && twauth.user_id && twauth.access_token_secret) {
      ...
      function(error, access_token_key, access_token_secret, params) {
        ...
        var screen_name = (params && params.screen_name) || null;
      ...
      var oauthTransaction = function(error, oauth_token, oauth_token_secret) {
        if (error) {
          // FIXME: do something more intelligent
          return next(500);
        } else {
          ...
          var tokenString = querystring.stringify({oauth_token: oauth_token});
          res.writeHead(302, {
            'Location': self.options.authorize_url + '?' + tokenString
          });
          res.end();
          return;
        }
      ...
      self.oauth.getOAuthRequestToken(oauthTransaction);
    } else {
...
Twitter.prototype.gatekeeper = function() {
  var self = this;
  var mount = this.options.login_mount || '/twauth';
...
Twitter.prototype.appAuth = function(callback) {
  ...
  var consumerPair = options.consumer_key + ':' + options.consumer_secret;
  request.post({
    url: options.authenticate_application_url,
    followAllRedirects: true,
    headers: {
      'User-Agent': pkginfo.name + '/' + pkginfo.version,
      'Authorization': 'Basic ' + new Buffer(consumerPair).toString('base64'),
...
Twitter.prototype.lookupUsers = Twitter.prototype.showUser;
...
Twitter.prototype.userProfileImage = function(id, params, callback) {
  ...
  var url = '/users/profile_image/' + escape(id) + '.json?' +
...
Twitter.prototype.showList = function(screen_name, list_id, callback) {
  screen_name = escape(screen_name);
  var url = '/' + screen_name + '/lists/' + escape(list_id) + '.json';
  this.get(url, null, callback);
  return this;
};

// FIXME: Uses deprecated Twitter lists API
Twitter.prototype.getListTimeline = function(screen_name, list_id,
                                             params, callback) {
  screen_name = escape(screen_name);
  var url = '/' + screen_name + '/lists/' + escape(list_id) + '/statuses.json';
  this.get(url, params, callback);
  return this;
};
...
// FIXME: Uses deprecated Twitter lists API
Twitter.prototype.createList = function(screen_name, list_name,
                                        params, callback) {
  screen_name = escape(screen_name);
  if (typeof params === 'function') {
    callback = params;
...
// FIXME: Uses deprecated Twitter lists API
Twitter.prototype.updateList = function(screen_name, list_id,
                                        params, callback) {
...
// FIXME: Uses deprecated Twitter lists API
Twitter.prototype.getListMembers = function(screen_name, list_id,
                                            params, callback) {
  screen_name = escape(screen_name);
  list_id = escape(list_id);
  if (typeof params === 'function') {
...
// FIXME: Uses deprecated Twitter lists API
Twitter.prototype.getListSubscribers = function(screen_name, list_id,
                                                params, callback) {
  screen_name = escape(screen_name);
  list_id = escape(list_id);
  if (typeof params === 'function') {
...
Twitter.prototype.updateDirectMessage = Twitter.prototype.newDirectMessage;
Twitter.prototype.sendDirectMessage = Twitter.prototype.newDirectMessage;
...
Twitter.prototype.newSavedSearch = Twitter.prototype.createSavedSearch;
...
Twitter.prototype.deleteSavedSearch = Twitter.prototype.destroySavedSearch;
...
Twitter.prototype.geoSimilarPlaces = function(lat, lng, name,
                                              params, callback) {
  if (typeof params === 'function') {
    callback = params;
    params = {};
  ...
  if (typeof lat !== 'number' || typeof lng !== 'number' || !name) {
    callback(new Error('FAIL: You must specify latitude, ' +
                       'longitude (as numbers) and name.'));
  }
...
Twitter.prototype._getUsingCursor = function(url, params, callback) {
  ...
  if (!key) {
    callback(new Error('FAIL: Results key must be provided ' +
                       'to _getUsingCursor().'));
  }
  delete params.key;
  ...
  function fetch(err, data, rate_limit) {
    ...
    var is_too_many_requests = (err && err.statusCode === TOO_MANY_REQUESTS);
